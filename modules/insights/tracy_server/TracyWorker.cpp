...
#include "../tracy_common/TracyProtocol.hpp"
#include "../tracy_common/TracySystem.hpp"
#include "../tracy_common/TracyYield.hpp"
#include "../tracy_common/TracyStackFrames.hpp"
#include "../tracy_common/TracyVersion.hpp"
...
{
    QueueZoneEnd unpack; unpack.time = int64_t( ev.time + ProtocolOffset16Bit );
    ProcessZoneEnd( unpack );
}

void Worker::ProcessZoneEnd16( const QueueZoneEnd16& ev )
{
    QueueZoneEnd unpack; unpack.time = ev.time;
    ProcessZoneEnd( unpack );
}
...
{
    QueueCallstackSample unpack = { ev.thread, int64_t( ev.time + ProtocolOffset16Bit ) };
    ProcessCallstackSample( unpack );
}

void Worker::ProcessCallstackSample16( const QueueCallstackSample16& ev )
{
    QueueCallstackSample unpack = { ev.thread, int64_t( ev.time ) };
    ProcessCallstackSample( unpack );
}
...
void Worker::ProcessSectionEnter( const QueueSectionEnter& ev )
{
    ...
    if( ait != m_data.sectionsActive.end() )
    {
        ...
        auto it = std::find_if( m_data.sectionsPending.begin(), m_data.sectionsPending.end(), [id = ev.id]( const auto& s ) { return s.start.Val() == id; } );
    ...
    {
        m_data.sections.push_back( SectionItem{ Int48(t), Int48(-int64_t(ev.id)), StringIdx(text) } );
...
void Worker::ProcessSectionLeave( const QueueSectionLeave& ev )
{
    ...
    {
        m_data.sectionsActive.erase( ait );
        auto it = std::find_if( m_data.sections.begin(), m_data.sections.end(), [id = -int64_t( ev.id )]( const auto& s ) { return s.end.Val() == id; } );
    ...
    {
        m_data.sectionsPending.push_back( SectionItem{ Int48(ev.id), Int48(t), StringIdx() } );
        m_data.sectionsActive.insert( ev.id );
    }
}
